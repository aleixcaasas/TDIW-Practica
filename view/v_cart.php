    ...
    <div id="pagCarrito">
        ...
        <table>
            <?php if (empty($_SESSION['cart'])) { ?>
                <tr>
                    <td id="msgCarrito" colspan='4'>Tu carrito está vacio</td>
                </tr>
            <?php } else { ?>
                <tr>
                    <th>Producto</th>
                ...
                <?php foreach ($row as $product): ?>
                    <tr>
                        ...
                        <td>
                            <button class="modifQuant"
                                    onclick="rmCamiseta(<?php echo $product[4] ?> , '<?php echo $product[1] ?>')"> -
                            </button>
                            <?php echo $product[1] ?>
                        ...
                        <td>
                            <button class="eliminarProducte" on onclick="rmProducte(<?php echo $product[4] ?>)"><img
                                        src="../img/icons/trash_icon.png"></button>
                        </td>
                    </tr>
                <?php endforeach; ?>
                <tr>
                    <td id="msgCarrito" colspan='4'>  <?php echo "El importe total es :  $totalPrice" ?>,00 €</td>
                </tr>
            <?php } ?>
        </table>
        <button id="botoPagCarrito" onclick="borrarCarrito()">VACIAR CARRITO</button>
        <button id="botoPagCarrito" onclick="tramitarPedido(<?php echo $mem[3] ?>, <?php echo $mem[1] ?>)">TRAMITAR
            PEDIDO
        </button>
    </div>
</div>
    ...
    async function tramitarPedido(nProductes, importTotal) {  //V_PRODUCTE
        if (nProductes > 0) {
            ...
            await fetch("controller/c_cart.php?process_order=" + nProductes + '&preu=' + importTotal)
                ...
                .then(data => {
                    document.getElementById("pagCarrito").innerHTML = data;
                });
        } else {
